...
/**
 * @title UsdGold Monetary Supply Policy
 * @dev This is an implementation of the UsdGold Ideal Money protocol.
 *      UsdGold operates symmetrically on expansion and contraction. It will both split and
 *      combine coins to maintain a stable unit price.
 *
 *      This component regulates the token supply of the UsdGold ERC20 token in response to
 *      market oracles.
 */
contract UsdGoldPolicy is Ownable {
    ...
    UsdGold public usdGoldC;

    // Provides the current CPI, as an 18 decimal fixed point number.
    IOracle public cpiOracle;

    // Market oracle provides the token/USD exchange rate as an 18 decimal fixed point number.
    // (eg) An oracle value of 1.5e18 it would mean 1 UsdGold is trading for $1.50.
    ...
    function rebase() external onlyOrchestrator {
        ...
        if (supplyDelta > 0 && usdGoldC.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY) {
            supplyDelta = (MAX_SUPPLY.sub(usdGoldC.totalSupply())).toInt256Safe();
        }

        uint256 supplyAfterRebase = usdGoldC.rebase(epoch, supplyDelta);
     ...
     */
    function initialize(address owner_, UsdGold usdGoldC_, uint256 baseCpi_)
    public
    initializer
    {
    ...
    returns (int256)
    {
        ...
        return usdGoldC.totalSupply().toInt256Safe()
