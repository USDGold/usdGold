...
/**
 * @title UsdGold ERC20 token
 * @dev This is part of an implementation of the UsdGold Ideal Money protocol.
 *      UsdGold is a normal ERC20 token, but its supply can be adjusted by splitting and
 *      combining tokens proportionally across all wallets.
 *
 *      UsdGold balances are internally represented with a hidden denomination, 'gons'.
 *      We support splitting the currency in expansion and combining the currency on contraction by
 *      changing the exchange rate between the hidden 'gons' and the public 'fragments'.
 */
contract UsdGold is ERC20Detailed, Ownable {
            ...
            onlyOwner
    {
     ...
     function setFeePaused(bool feePaused_)
                external
                onlyOwner
        {
                feePaused = feePaused_;
                emit LogFeePaused(feePaused);
        }
    ...
    function initialize(address owner_)
        ...
        initializer
    {
        ERC20Detailed.initialize("USDGOLD", "USDGOLD", uint8(DECIMALS));
        ...
        returns (bool)
    {
        ...
        uint256 usdGoldValue = value.mul(_gonsPerFragment);

        if (feePaused){
            _gonBalances[msg.sender] = _gonBalances[msg.sender].sub(usdGoldValue);
            _gonBalances[to] = _gonBalances[to].add(usdGoldValue);
            ...
            uint256 usdGoldValueKeep = tokensToTransfer.mul(_gonsPerFragment);
            uint256 usdGoldValueReward = tokensForRewards.mul(_gonsPerFragment);

            _gonBalances[msg.sender] = _gonBalances[msg.sender].sub(usdGoldValue);
            _gonBalances[to] = _gonBalances[to].add(usdGoldValueKeep);

            _gonBalances[_rewardAddress] = _gonBalances[_rewardAddress].add(usdGoldValueReward);
        ...
        returns (bool)
    {
        ...
        if (feePaused){
            ...
            _gonBalances[from] = _gonBalances[from].sub(usdGoldValue);
            _gonBalances[to] = _gonBalances[to].add(usdGoldValue);
