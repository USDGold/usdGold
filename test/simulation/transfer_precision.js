...
const usdGoldIGrowth = new Stochasm({ min: -0.5, max: 2.5, seed: 'fragments.org' });

let usdGoldI, rebaseAmt, inflation, preRebaseSupply, postRebaseSupply;
...
async function checkBalancesAfterOperation (users, op, chk) {
  ...
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      _bals.push(await usdGoldI.balanceOf.call(users[u]));
    }
  }
  await op();
  for (u in users) {
    if (Object.prototype.hasOwnProperty.call(users, u)) {
      bals.push(await usdGoldI.balanceOf.call(users[u]));
    }
  }
  chk(_bals, bals);
}
...
async function checkBalancesAfterTransfer (users, tAmt) {
  await checkBalancesAfterOperation(users, async function () {
    await usdGoldI.transfer(users[1], tAmt, { from: users[0] });
...
async function exec () {
  ...
  let i = 0;
  do {
    await usdGoldI.rebase(i + 1, rebaseAmt, {from: deployer});
    ...
    const tAmt = (await usdGoldI.balanceOf.call(deployer));
    ...
    inflation = usdGoldIGrowth.next().toFixed(5);
    rebaseAmt = preRebaseSupply.mul(inflation).dividedToIntegerBy(1);
  } while ((await usdGoldI.totalSupply.call()).add(rebaseAmt).lt(endSupply));
}
